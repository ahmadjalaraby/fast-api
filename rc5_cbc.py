...
def RC5_CBC_encryption(msg, key, block_size = 128, rc5_rounds = 18):
    ...
    blocks_to_process = [secrets.randbits(128)] + ['' for n_blocks in range(math.ceil(len(msg) / block_byte_size))]

    for i in range(len(msg)):
        blocks_to_process[1 + i // block_byte_size] += msg[i]

    stolen_length = block_byte_size - len(blocks_to_process[len(blocks_to_process) - 1])

    blocks_to_process[len(blocks_to_process) - 1] += '\0' * stolen_length
...
def RC5_CBC_decryption(encrypted_blocks, key, stolen_length, block_size = 128, rc5_rounds = 18):
    ...
    # returns plaintext message in string format
    ...
    for i in range(len(decrypted_blocks)):
        ...
        for j in range(block_size // 8):
            ...
            block_msg += chr(decrypted_blocks[i] & 255)
...
s_k = RC5_key_generator(secrets.randbits(1024))
print(s_k)


t = RC5_CBC_encryption('Just this text', s_k)
print(RC5_CBC_decryption(t[0], s_k, t[1]))